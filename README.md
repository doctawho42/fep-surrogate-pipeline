...
make paper    # generic build         -> docs/paper_draft.pdf + docs/paper_draft_si.pdf
```
...
separate file, so each target produces two: the manuscript, and an S-numbered Supporting
Information with its own title page. The two reference each other's figures and sections through
`xr`, which reads the other document's `.aux`, so each target builds them in alternating passes
