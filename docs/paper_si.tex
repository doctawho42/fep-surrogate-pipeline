...
out-discriminates a fixed $1.0$ kcal/mol hysteresis cutoff (AUC $0.865$ against $0.575$; paired bootstrap difference $+0.290$ $[+0.094,+0.490]$, excluding zero) but not a fixed-$\mathrm{se}$ $\chi^2$ test (AUC $0.725$; $+0.140$ $[-0.008,+0.316]$). The pre-registered criterion required the calibrated null to beat both, so it returns a tie; the gain over the cutoff the field actually uses is nonetheless real. P2 tested whether
...
freedom versus at the shared-node-correlation-corrected effective degrees of freedom, with the $y{=}x$ line and the $\chi^2_\nu=1$ reference. The standard-error factor annotated in panel B is the closure-implied functional whose premise was subsequently found unsound; it is retained as the pre-registered record and is not a conservatism estimate.}
\label{fig:Cut}
\end{figure}
...
alone, so the three $q$ columns are not a joint error rate across replicates. All values are produced by the analysis code released with this work .
...
(B) Held-out inconsistency of edges selected on one replicate and evaluated on the other two,
against an $|z|$-rank-matched control, a topology-only control built from curl-leverage, and
equal-size random selection, over the three choices of selection replicate.}
\label{fig:OOS}
\end{figure}
...
the nominal test, which uses the reported standard errors unchanged; the pre-specified primary,
which corrects each edge by its own ratio; a coarser variant correcting each system by its
aggregate ratio; a variant clipping the ratio to $[1/3,3]$; a variant dividing the replicate
standard deviation by $c_4(3)=0.886$, the expectation of the sample standard deviation at three
draws, which widens the bars; and the one-sided correction of Table~\ref{tab:P8}, which inflates
only where the measurement says the bar is too tight.}
\label{fig:Self}
\end{figure}
...
a system. A system enters the comparison only if at least $60\%$ of its network ligands map to a
measured value. Table~\ref{tab:chembl} lists what was used.

\begin{table}[t]\centering
\caption{\textbf{ChEMBL provenance of the experimental affinities.} Target identifier, protein,
assay type, and the number of network ligands with a measured value, for each system that clears
the $60\%$ coverage requirement. Affinities are converted at $RT\ln 10 = 1.364$ kcal/mol and
