...
structures.} Heavy-atom coordinates. Colours here annotate structural objects, not methods.
...
both, so the panel records a tie. No ordering inside the panel may be read as a discrimination
gain: the scored label is standardized by the same per-edge reported variance the calibrated rule
uses as its weight, so the comparison is contaminated. (B) Per-system reduced $\chi^2$ at the nominal degrees of
