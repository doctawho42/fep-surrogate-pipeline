...
the quality-control results (Figures~\ref{fig:Lval} and~\ref{fig:L} of the main text and
...
\paragraph{Priority: what the decomposition owes to prior work.} The main text states that the
decomposition is not new. This is the accounting behind that, kept here because it is a claim
about the literature rather than about the benchmark.
...
\paragraph{The H1 criterion, per system and as pooled.} The pooled degrees-of-freedom-adjusted
gradient-model $R^2$ the main text reports as $0.980$ is $0.978$, $0.995$, $0.976$ and $0.761$ on
the four systems taken singly. Pooling is over the four sub-networks jointly, $f$ as a ratio of
summed squared norms, with the adjustment carried on the summed $E=167$ edges and rank $116$, that
is on $51$ residual degrees of freedom, and not as an average of per-system adjustments. The form
evaluated is the degrees-of-freedom-adjusted rule the pre-registration froze, and not the weaker
unadjusted $1-f_{\text{pooled}}\geq0.50$; the two differ and the frozen one is the harder.
Contaminant checks: the reading holds on medians rather than sums, and holds on \texttt{bace},
whose ligands carry no enantiomer pair collapsed by the stereo-blind join.
...
\paragraph{The four calibration functionals, and why they disagree (Figure~\ref{fig:Cal}).}
