...
adjacent-$\lambda$ BAR windows (mean $6.07$, $[2.78,12.01]$), the worst overstatement occurring at
\emph{high}
...
which windows happened to be sampled. Across the full $19$-window set the trend is not an
...
common $\lambda$ schedule. Every row is a two-state estimate between neighbouring $\lambda$
values, not a ligand-to-ligand binding free energy.}
\label{tab:bace1}
\small
\begin{tabular}{@{}rrrrr@{}}
\toprule
...
therefore a property of the per-edge \emph{parameterisation} rather than of the label budget or of
...
$4000$ edges ($\sim$4\% at $40000$) on the same labels the heads were given. That matters for what
the quality-control comparison can be read to show. Placing a $6\%$ miscalibration on the frozen
scale grid of Figure~\ref{fig:P4b} gives seven flagged systems of $48$ against the shipped six, and
...
\paragraph{Gauge-aware acquisition (Figure~\ref{fig:D}).} A controlled simulation
...
where the gauge-aware one reaches $0.01$.
 ...
 The Fisher--resistance Laplacian
...
detectable shift over that system's auditable edges. Three readings are worth making explicit.
The auditable fraction is exactly $1-(N-c)/E$ and so is small wherever a network is sparse: it
...
from $0.12$ to $0.46$, rather than sitting at the auditable end, as they should: a system is
flagged for the error it carries, not for how much of that error is in view. And \texttt{brd4}, flagged on two of the three replicates, sits
...
\paragraph{The pre-registration, in full.} Four separate arguments in this article turn on what was
frozen before the computation rather than chosen after it, so the file is reproduced below rather
than described. It was committed to the article's repository before the commit that computes any of
the quantities it governs, and it is anchored by the SHA-256 digest
\texttt{16279af73d90e1e9b39f33046ba6ee432b80ad750a6e5687a0e05b3e54b01585}, which the test suite
asserts on every run so that an edit after the fact fails the build. Note in particular that both
success criteria are conjunctive, that H1 is labelled a quantification rather than a gamble and
says why, that the confound the design cannot resolve is declared in it, and that H2 is declared a
second look at data which had already produced one null. The quantities H3 lists as descriptive
carry no threshold and no verdict.
\scriptsize
\verbatiminput{../data/openfe_replicates/hodge_prereg.yaml}
\normalsize
...
statistic itself uses. Three readings of the same four sub-networks are collected here. In the
whitened metric the visible fractions are $0.0065$, $0.0017$, $0.0086$ and $0.0460$ for
\texttt{cdk8}, \texttt{hif2a}, \texttt{p38} and \texttt{bace}, against the $\mathrm{dof}/E$ chance
levels $0.290$, $0.322$, $0.353$ and $0.192$. Under isotropy in kcal/mol the chance level is
$\mathrm{tr}(\Pi W)/\mathrm{tr}(W)$ instead, giving $0.127$, $0.051$, $0.124$ and $0.092$, so the
margins narrow to roughly $20$, $30$, $14$ and $2$ times, the last being the weakest and the one
the main text quotes. Measured in the unweighted metric the pooled fraction rises from $0.6\%$ to
$5.45\%$, with \texttt{p38} at $0.137$ against its $0.353$, a factor of $2.6$. Leaving out one edge
at a time moves the per-system whitened values within $0.003$--$0.011$, $0.001$--$0.003$,
$0.003$--$0.013$ and $0.003$--$0.063$. Recomputing the ratio on medians rather than sums, which
...
The component column is not decorative. Six of the $48$ systems are disconnected
...
rank already accounts for the extra kernel direction. Two things do change. The offset that
Theorem~\ref{thm:hodge}(iv) says the first anchor removes is one offset \emph{per component}, so a
disconnected system needs one anchor in each piece before any further anchor buys a dimension. And
the grounded networks of Section~\ref{sec:hodge} inherit the same structure, \texttt{cdk8} and
...
more experimental information than a prediction is entitled to, lowers the error by $0.067$ and
...
$\mathrm{MUE}(\text{full})-\mathrm{MUE}(\text{after})$, so a positive value is an improvement,
the same orientation as Figure~\ref{fig:Hodge}C. Systems: \texttt{cdk8},
...
scores $0.865$ against a fixed $1.0$ kcal/mol hysteresis cutoff's $0.575$ (paired bootstrap difference $+0.290$ $[+0.094,+0.490]$) and against a fixed-$\mathrm{se}$ $\chi^2$ test's $0.725$ ($+0.140$ $[-0.008,+0.316]$, straddling zero). The pre-registered criterion required the calibrated null to beat both, so the verdict is a tie. We withdraw the reading that the ordering within that tie is nonetheless a real gain over the cutoff the field uses: the scored label standardizes each residual by the same per-edge reported variance the calibrated rule weights by, so the AUC ordering is monotone in a rule's proximity to that variance and the comparison ranks nothing. The second check tested whether
...
$20$ of its flags. Whether that removal is an improvement is exactly what the contaminated
comparison above cannot say. The $1.0$ kcal/mol cutoff was pre-registered at the
...
both, so the panel records a tie. The panel is annotated with the reason no ordering inside it is
read as a discrimination gain: the scored label is standardized by the same per-edge reported
variance the calibrated rule uses as its weight. (B) Per-system reduced $\chi^2$ at the nominal degrees of
...
synthetic null, against the nominal $0.05$; that simulation is specified in Computational details
(Section~\ref{sec:methods} of the main text). No arm is reported as a flag set.}
\label{tab:selfgrid}
\small
...
it: the raw ratios used here are if anything generous to those systems, not the reverse.
Deterministic; the full $48$-system table is released with the analysis code.
