...
neither threshold nor verdict. Four properties of it matter for reading the results above. Both
...
licenses is the first of those and not the second. And H2 is declared a second look at data that
had already produced one null, so its result is reported with that status attached.
...
left. Three criteria are recorded in the analysis code and not in the design file. The
