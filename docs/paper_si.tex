...
\paragraph{The differentiable backward pass.} The BAR layer's backward pass is quoted in the main
text as costing one division. The statement is recorded here rather than there because no
experiment in this article trains through the layer, so the result prices a capability rather than
underwriting a measurement.
...
\paragraph{The pre-registration, in full.} Several arguments in this article turn on what was
...
\texttt{16279af73d90e1e9\allowbreak{}b39f33046ba6ee43\allowbreak{}2b80ad750a6e5687\allowbreak{}%
a0e05b3e54b01585}, which the test suite asserts on every run. That digest lives in the same
repository as the file, so it detects accidental drift and not backdating; we claim the former and
not the latter. The file's own header says the immutability is anchored by an \emph{external}
digest and calls itself immutable; neither is right and the prose here governs. There is no
third-party timestamp, no external registry entry and no notarisation of any kind: the anchor is
in-repository, its only guarantee is drift detection, and the file's wording is left as frozen
rather than corrected because correcting it would change the digest. Three labels in the frozen file
are also wrong and are left standing for the same reason. It calls the influence statistic
$|z_e|\sqrt{(1-h_e)/h_e}$ the DFFITS form, which it is not: it is the classical influence form
scaled by $\sqrt{h_e}$. It cites ``Theorem 5'' for the conserved budget, where the conservation law
is Theorem~\ref{thm:cons} and Theorem~\ref{thm:hodge} is the decomposition; its numbering also
predates the move of the backward-pass theorem into this supplement, so every theorem number it
quotes is one higher than the article's. And it cites a part ``5(i')'' that does not exist, the
intended reference being part (i) of Theorem~\ref{thm:hodge}. The file below governs
H1, H2 and H3 only. The other pre-specified constants this article invokes, the $|z|$-rule
...
granularity of the self-calibrated null, and the close-the-loop criterion, are recorded in the
released analysis code and its accompanying pre-registration for that experiment, not here. Note in particular that both
...
\paragraph{Audit of the frozen criteria that live only in code.} One mismatch between a frozen
criterion and its printed description was found by review, so the rest were checked rather than
left. Three criteria are recorded in the analysis code and not in the file above. The
...
\emph{both} baselines, implemented as a paired bootstrap of
...
result depends on the discrepancy, the file is left frozen, and the description printed in this
article is the one the code implements.
...
always one released data set: eight of them union two or more, which the paragraph below takes up.
\texttt{ciordia\_retro} is released under the \texttt{asap\_mix} group and we do not assert a target
for it beyond what the release states.
...
for P2 to explain, and we do not revise the pre-registered constants after the fact. On replicate $0$ the calibrated null flags $6$ of $48$
...
nests, and the calibrated null removes $23$ of the cutoff's flags. Whether that removal is an improvement is exactly what the contaminated
...
systems. Benjamini--Yekutieli~\citep{benjamini2001by} needs none, at the price of running the same
...
not depend on the dependence assumption, and the churn across replicates is unchanged by removing
it. \texttt{make figStab} emits the three sets.
...
synthetic null, against the nominal $0.05$, with its exact binomial interval, since at this draw
count a realized $0.003$ is one flag and is not resolved from zero; that simulation is specified in Computational details
...
That second arm is a counterfactual stand-in, the miscalibration measured for a trained
per-edge head in Figure~\ref{fig:A} imposed on the assumed error, and not an estimator run
here; Table~\ref{tab:gsweep} gives the stopping decision across the whole scale.
...
(Table~\ref{tab:gsweep}).} Figure~\ref{fig:G} reports one point of a curve. What the
experiment measures is how the stopping decision depends on the error the learner
\emph{assumes}, everything else held fixed, so the whole dependence is reported here. The
two arms of the figure keep their own Monte-Carlo stream, so adding the sweep points leaves
them unchanged.
...
estimator would stop early. The one effect that survives a fair comparison runs the other
way: a $6\%$ over-estimate spends $30.8$ calls for the same correctness, the decision cost of
...
counterfactual stand-in both lose heavily, which prices miscalibration, while against a fair
...
gone (Figures~\ref{fig:J}B, \ref{fig:G} and Table~\ref{tab:gsweep} above).
...
\midrule
Commit trust vs overconfident $\sigma$ & real & shortfall $+0.182$ $[0.132,0.225]$ better & prices miscalibration \\
...
Stopping vs $0.15\times$ stand-in & sim & $82\%$ vs $60\%$ correct at stop & prices miscalibration \\
