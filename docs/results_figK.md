...
True-hit-rate (12 seeds): calibrated-LCB 0.332, raw-μ̂ 0.336,
MVE-σ 0.273. cal−raw -0.004, CI [-0.026, +0.017]; cal−mve
+0.059, CI [+0.023, +0.104] -> **KILL** (calibration wins iff BOTH CI
lower bounds > 0).

## Honest negative — the GLS ceiling (third manifestation)
On p38 the calibrated reward does NOT improve generation: cal is statistically indistinguishable from raw (cal-raw CI [-0.026, +0.017] straddles 0). The cause is the same
...
generation — untested. Reproducibility: the means and CIs shift modestly run-to-run (torch CPU non-determinism +
bootstrap over 12 seeds), so the cal-raw CI boundary is noisy — but the verdict (cal does NOT exceed
