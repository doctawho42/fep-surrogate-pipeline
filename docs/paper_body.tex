\begin{abstract}
A thermodynamic cycle in a perturbation network must close, and how far it fails to close is
routinely reported as a diagnostic without a scale to judge it against. We supply that scale from
the calculation itself. The Bennett Acceptance Ratio estimator that turns simulation output into a
free energy carries its own finite-sampling variance in closed form, and the per-edge error bar a
practitioner already receives from that family of estimators is checked here against independent
molecular-dynamics replicates on a large set of real protein-ligand binding edges. Used as the
null for cycle closure, it converts the check into a test that flags a selective minority of
networks. Those replicates share their starting coordinates, so a flag establishes inconsistency
reproducing beyond sampling noise rather than physical error, and the ranking behind the flags is
steadier than the list of names flagged at any fixed threshold. Miscalibration measured on one set
of replicates predicts the closure statistic of a replicate it never saw, which shows how much of
that statistic is a readout of how well the bars are scaled. An overconfident stand-in for the
error bar destroys the test, flagging almost every network. The same fit says in advance which
edges of a planned network can carry evidence at all and how large an error each would need before
closure could see it. Audited across further uses, the calibrated bar sharpens no interval,
changes no ranking and improves no reward-guided re-ranking. Its demonstrated value on real data
is detection; trust and stopping pay in controlled simulation that does not consume the physics
variance.
\end{abstract}
...
\texttt{eg5}); it pays on commit trust and on stopping in simulations that do not consume the
physics variance; and an \emph{overconfident} $\sigma$
...
The instrument measures internal consistency, which is not agreement with experiment; acting on
its flags repairs the first and leaves the second where it was. What it delivers on real data is
an ordering of networks by how far their own calculations disagree with each other, together with
a stated operating point and a map of what that operating point cannot reach. Trust in a commit
decision and a stopping rule for an acquisition loop both pay in simulation, and neither is
exercised here on the variance the rest of this paper is about. The ordering costs nothing beyond
the calculation already being run, which is the argument for reading uncertainty off the physics
