\begin{abstract}
The per-edge error bar a relative-binding-free-energy pipeline already reports, checked here
against seed and velocity repeats on $1145$ real protein-ligand binding edges, is conservative on
aggregate and heterogeneous by system; used as the null of a cycle-closure fit it turns a
descriptive consistency check into a test with a stated null. What that test can reach is fixed by
the gradient--cycle decomposition of edge flows, which is not ours and is attributed: the cycle
part of a network's error is identifiable from its own edge values and biases nothing, while the
gradient part biases the estimated affinities and is invisible to any statistic computed from those
values, at any magnitude. Measured on this benchmark the auditable subspace spans a third of the
error space and carries under one per cent of the error against experiment on the systems grounded
against public affinity data, below chance on every one of them. The test is accordingly not a
repair list: on the first replicate its flags lie strictly inside the fixed $1.0$ kcal/mol
hysteresis cutoff the field uses, catching nothing that cutoff misses while removing $20$ of its
$26$, a difference retrospective data cannot adjudicate, and removing the edges it names does not
recover accuracy against experiment. Resolving the invisible part needs measurements from outside
the network, priced here in ligands.
\end{abstract}
...
the per-edge residuals and the ranking of systems reproduce across independent repeats, the list of
flagged systems does not, and a bar as overconfident as a learned one destroys the separation.
