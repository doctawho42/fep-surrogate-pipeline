...
(\texttt{merck\_cdk8} $p=0.218$, $0.865$, $0.189$ on the three replicates). \texttt{cdk8} is
...
system is accurate: across the five grounded systems the flag does not separate accuracy against
experiment, \texttt{eg5} at $0.79$ kcal/mol tying the flagged \texttt{p38} at $0.79$ and beating
only two of the four flagged systems (\texttt{cdk8} $0.91$, \texttt{bace} $0.88$, \texttt{hif2a}
$1.47$). The comparison is also not at matched grounding: \texttt{eg5} comes from a single named
ChEMBL assay while the four flagged systems come from a target-wide connectivity search under the
coverage rule. Acting on the QC does \emph{not} improve accuracy here (internal $|z|$ vs
