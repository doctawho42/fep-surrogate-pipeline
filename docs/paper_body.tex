...
negatives are the reason to believe the positive result. Four performance tasks (sharpness, edge
ranking, commit-to-synthesis selection, and reward re-ranking over a measured congeneric set,
single target \texttt{eg5}) run on the public OpenFF protein--ligand
benchmark~\citep{hahn2022plbench} (FEP+ and Merck data~\citep{wang2015fep,schindler2020merck};
experimental $\ddG$ labels, both-endpoint scaffold-disjoint out-of-distribution (OOD) splits);
two decision tasks run in controlled simulations, and Supplementary Table~\ref{tab:audit}
collects every effect against its baseline. The amortized $\sigma$ under test (the decomposed
...
\emph{lowers} the true hit-rate of the sampled distribution ($0.140$ vs raw $0.250$ and MVE
$0.256$; cal$-$raw $-0.110$, CI $[-0.151,-0.073]$; Supplementary Figure~\ref{fig:K}). The
...
\paragraph{Synthesis.} Five independent tests agree: a calibrated per-edge $\sigma$ does not
sharpen intervals on real data, does not re-rank what to compute or commit, and does not steer
reward re-ranking toward better molecules on this evaluable congeneric set (single target
\texttt{eg5}); it pays on stopping; and an \emph{overconfident} $\sigma$ silently corrupts the
