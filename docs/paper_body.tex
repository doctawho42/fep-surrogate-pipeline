...
the work samples rather than learned. The per-edge error bar a practitioner receives from that
family of estimators is validated against independent molecular-dynamics replicates on a large set of real
...
test: it flags 42 of the 48 networks where the calibrated bar flags 6, so almost nothing is left
unflagged for a decision to act on. The detector
...
Across the 1143 real binding edges that lie in the 48 benchmark systems carrying a cycle (of 1145
edges over 49 systems), a calibrated error bar flags 6, 6 and
3 systems on the benchmark's three protocol repeats, one of them on all three; an overconfident
stand-in built from a learned head's measured miscalibration profile flags 42.}
\label{fig:overview}
\end{figure}
...
of the network can carry evidence at all. The instrument consumes the variance as a plain number.
Packaging the same closed form as a differentiable layer, at the cost of the point estimate, is
what carries the quantity into a surrogate and into an acquisition rule; the experiments reported
here consume it as a number and as a graph weight.
...
cost of the point estimate. The experiments reported here consume it as a number and as a
graph weight.
...
worse in the complex leg than in the ligand leg. Two things bear on the disagreement, and they point in
opposite directions. The first is what is being repeated: their replicas resample the full
ensemble, whereas the OpenFE repeats used here share starting coordinates and differ only in
stochastic sampling, so the denominator we divide by is a lower bound on run-to-run variability
and the resulting ratio is an upper bound on conservatism. The second is what happens when their
comparison is run on these edges rather than ours. They report how often the analytic standard
error falls below the replicate spread; here that happens on $0.279$ $[0.224,0.332]$ of edges,
against the $0.368$ that correctly scaled bars produce at three replicates, and the interval
excludes it. On the functional they actually measure, these bars are conservative rather than
optimistic, which sharpens the disagreement instead of resolving it. What remains of a
reconciliation is the first difference alone, and settling the question needs replicates that
resample preparation and pose, which this benchmark does not provide.
...
Theorem~\ref{thm:fr} (Section~\ref{sec:gauge}) identifies
...
cycle graph itself. On the same $1143$ edges, and scored against the reproducible-systematic
magnitude of Section~\ref{sec:methods}, flagging by the calibrated per-edge null
out-discriminates the fixed $1.0$ kcal/mol hysteresis cutoff the field uses (area under the
receiver-operating-characteristic curve, AUC, $0.865$ against $0.575$; paired bootstrap difference
$+0.290$ $[+0.094,+0.490]$, excluding zero) but not a fixed-$\mathrm{se}$ $\chi^2$ test built on
the same statistic (AUC $0.725$; $+0.140$ $[-0.008,+0.316]$, straddling zero; Supplementary
Figure~\ref{fig:Cut}). The pre-registered criterion required beating both, so the conjunctive
verdict is a tie. What this benchmark demonstrates is therefore a discrimination gain over the
fixed cutoff and none over the fixed-$\mathrm{se}$ test, and the calibrated null's remaining
advantages are structural, namely per-edge-adaptive thresholds, a false-positive rate tied to a
replicate-validated null, and the observability map of Theorem~\ref{thm:cons}. The evidence that
the flags are real remains the distribution-free cross-replicate
...
the \emph{calibrated null}, not the differentiable machinery. That null is conservative in the bulk, with a median reduced $\chi^2$ of $0.34$, so on a typical system the test under-flags.
...
system's own null by the reciprocal of its own measured (raw) ratio, one pooled ratio per system
(Supplementary Table~\ref{tab:P8}), leaves all six flags in place, each self-calibrated reduced
$\chi^2$ ($2.36$--$8.97$) remaining far above the level a sampling-consistent network of this size
and degree-of-freedom profile shows. That per-system pooling is not, however, the granularity this
work pre-specified for a self-calibrated null. The pre-specified granularity is \emph{per edge},
each edge corrected by its own replicate-measured ratio, and the same one-sided inflation applied
at that granularity leaves \emph{two} of the six, \texttt{bace} and \texttt{cdk8}. Both
granularities are reported side by side in Supplementary Table~\ref{tab:selfgrid}, with their
two-sided counterparts and with each arm's realized probability of a false flag under a
pure-null simulation; among the arms whose realized level was measured, the pre-specified per-edge
one-sided arm is the only self-calibrated one that holds it ($0.003$ against a nominal $0.05$), so
its count of two is the conservative answer to how many of the six survive being retested against
the width their own replicates measure.
...
rates: which of the flags are false is exactly what is not known here (Section~\ref{sec:limitations}). That
...
which occurs only for reproducible error (single-replicate
$\chi^2_\nu=1+\sigma^2_{\text{sys}}/V_{\text{samp}}$ vs
pooled $1+3\sigma^2_{\text{sys}}/V_{\text{samp}}$, with $\sigma^2_{\text{sys}}$ the systematic
...
replicates ($r=+0.30$ to $+0.42$, $n=1143$, each interval excluding zero under a bootstrap over systems; Figure~\ref{fig:Lval}B), so what the flags identify is reproducible across runs rather than a property of one draw. Sampling noise would give zero.
...
model that turns FEP cycle-closure into a quality-control test that separates systematic from sampling error on real networks and whose output reproduces on replicates it never saw, the sharpest use of the free calibration, and one an overconfident
...
The churn has a structural source worth stating. The number of independent cycles is small on
much of this benchmark: nine of the $48$ systems carry exactly one, twenty-three carry three or
fewer, and the quartiles are $2$, $5$ and $12$. A reduced $\chi^2$ on one or two degrees of
freedom is an unstable statistic, and the instability is concentrated exactly there: the median
swing across the three repeats is $14.2\times$ for the single-cycle systems against $3.9\times$
for the rest. The flags themselves come from the other end of that distribution, seven of the
eight systems ever flagged and the one flagged on every repeat being well-connected, so the churn
is largely the low-cycle tail moving in and out rather than the detector disagreeing with itself
where it has power.
...
$\sigma_{\text{total}}=\gamma\sqrt{\sigma^2_{\text{ens}}+\sigma^2_{\text{ale}}}$: an
epistemic ensemble spread $\sigma_{\text{ens}}$ and a benchmark aleatoric floor
$\sigma_{\text{ale}}$, conformally scaled by a scalar $\gamma$ at miscoverage $\alpha=0.1$
(90\% target coverage; $\gamma$ rather than $q$, which is reserved throughout for a
Benjamini--Hochberg adjusted $p$-value). It is a different
...
The three \emph{signed} residuals of an edge are averaged, $\bar z_e=\frac13\sum_k z_e^{(k)}$, so
...
only replicate residuals and no detector output, although it shares the standardized-residual
construction with two of the three rules being compared. ``Edges whose sign and magnitude
...
samples, the absolute difference of the two means, and the normalized overlap scalar. The last of
these is what makes the foil fair, since it gives the head the same overlap information the closed
form uses. The network is a multilayer perceptron with two hidden layers of $32$ units and SiLU
...
work samples each; the large-budget arm raises the edge count and the corrected arms replace the
...
heterogeneous per system ($28$ of the $34$ systems with at least eight edges conservative; one
protonation-variant outlier overconfident). The three replicates are seed/velocity-only repeats from identical starting
...
should be tested across more chemotypes. Point estimates also shift modestly run-to-run (torch CPU non-determinism), so the single-target re-ranking result is directional. The cycle-closure quality-control test (Section~\ref{sec:qc}) detects only \emph{edge-level} inconsistency (node-consistent force-field bias cancels around a loop and is invisible to it), and its flags are validated retrospectively against held-out replicates and against experiment both on one clean system (\texttt{eg5}, where acting on the flags neither helps nor harms) and, now, on four flagged systems grounded against public ChEMBL affinity (\texttt{cdk8, hif2a, p38, bace}), where guided QC-edge removal ties random removal on MUE-vs-experiment (combined Stouffer $p=0.48$; Figure~\ref{fig:Lcausal}), confirming the same scope boundary on real, flagged, multi-system data: what the QC has been shown to reach is internal edge-level consistency, not force-field-limited absolute accuracy. This retrospective test is powered only by $K=1$--$3$ guided removals per system and by a connectivity-based ChEMBL join (stereo-blind, so enantiomer pairs sharing a 2D graph collapse to one experimental value, inflating absolute MUE but canceling in the guided-vs-random contrast since both arms read the identical cached affinity); a real but small effect could be underpowered at this $n=4$. The fully prospective flag$\to$repair$\to$remeasure test on a flagged system, with new MD rather than a retrospective affinity join, lies outside what public data can support. The distribution-free cross-replicate check establishes that what is flagged reproduces across independent runs, but it is not a quantitative false-positive-rate guarantee: it does not certify the nominal Benjamini--Hochberg rate itself. The detector's \emph{recall} cannot be measured directly, since there are no ground-truth error
labels. Replacing each system's reported standard error by its own replicate-measured
calibration, applied in whichever direction the measurement points rather than only the
conservative one, changes the flag set in both directions and by an amount that depends on the
granularity of the correction. Applied two-sided at the pre-specified per-edge granularity the
count goes from six to seven, three systems entering and two leaving; correcting instead by each
system's aggregate ratio takes it to sixteen with none leaving (Supplementary
Figure~\ref{fig:Self}); applied one-sided, the direction the circularity concern of
Section~\ref{sec:qc} calls for, the per-edge arm retains two of the six and the per-system arm all
six (Supplementary Table~\ref{tab:selfgrid}).
Neither two-sided arm is false-discovery-rate controlled at three replicates, and no arm is
reported as a flag set: rescaling an edge by its own replicate spread multiplies the statistic by
$\sigma^2/s^2$, whose expectation $\nu_{\text{rep}}/(\nu_{\text{rep}}-2)$ is undefined at the
$\nu_{\text{rep}}=2$ replicate degrees of freedom that three replicates provide ($\nu_{\text{rep}}$,
not the cycle count $\nu$ of the closure statistic), and a pure-null simulation puts the probability of at least one false
flag at $0.59$ for the two-sided per-edge arm and $0.24$ for the two-sided aggregate one, against
a nominal $0.04$; the one-sided per-edge arm is the only self-calibrated arm that holds its level
($0.003$).
What the experiment establishes is that the flag set is sensitive to calibration error of the size
actually present, in both directions, which is a limit on how far any single flag list should be
trusted. Together with the median reduced $\chi^2=0.34$ (Section~\ref{sec:qc}), which implies
limited single-replicate power, this makes the flagged systems a floor on the systems carrying
systematic error rather than a prevalence estimate: on the same replicate data, \texttt{renin} is systematic by the pooled-replicate ratio (reduced $\chi^2$ rising to $4.7$ when the three replicates are pooled, from a per-replicate mean of $2.5$, the same pooled-ratio signature Panel~C uses to confirm the six flagged systems; ratio $1.9$, between the sampling-only pole at $1$ and the systematic pole at $3$) yet did not clear FDR on a single replicate, a documented systematic-but-unflagged case. \texttt{ciordia\_retro} is a second such case, flagged on one of the three replicates and retained when they are pooled, and it is not otherwise discussed in this paper. The error runs the other way too, and one member of the published six should be read with more caution than the rest. \texttt{faah} clears FDR on the first replicate alone; its reduced $\chi^2$ crosses one in both directions across the three ($3.71$, $0.45$, $1.47$), its standardized residuals essentially fail to reproduce between replicates ($r=-0.085$, the weakest of the eight systems ever flagged), it is the single system the pooled fit drops, and in the held-out test of Section~\ref{sec:oos} it moves against the direction the other flagged systems move in. We therefore report \texttt{faah} as a flag that a second replicate does not support, rather than as a member of the set on the same footing as \texttt{bace}, which is flagged on every replicate and by every criterion here. The amortized
...
depends on them. Finally, a structure-based reverse screen was tried on a separate ten-target diverse-fold panel,
docking each scaffold-disjoint held-out ligand into all ten pockets and ranking targets by score.
It recovers the true target at top-$1$ $0.16$ (area under the curve $0.62$), against $0.95$
($0.99$) for the ligand-shape baseline of Supplementary Figure~\ref{fig:F} and $0.10$ for random,
raw docking scores not being comparable across pockets; the structure-directed regime is
accordingly not claimed here.
