...
family of estimators is validated against independent molecular-dynamics replicates on a large set
of real protein-ligand binding edges and found trustworthy against sampling and convergence noise.
Its sharpest use is detection: fitting cycle closure with these calibrated bars turns an ordinary
consistency check into a quality-control test that flags a selective minority of perturbation
networks. Miscalibration measured on one set of replicates predicts the closure statistic of a
replicate it never saw. Those replicates share their starting coordinates, so what is established
is inconsistency reproducing beyond sampling noise rather than physical error, and the ranking
behind the flags is steadier than the list of names flagged at a fixed threshold. An overconfident
stand-in for that error bar, built from a learned head's measured miscalibration rather than by
running the head here, destroys the test: it flags almost every network, leaving nothing
unflagged to act on. The detector consumes the variance as a plain number; the same closed form is
differentiable at a backward pass no costlier than the forward pass and yields an acquisition
weight insensitive to gauge-redundant directions. Audited further, the calibrated bar sharpens no
interval, changes no ranking and improves no reward-guided re-ranking; its value is confined to
detection, commit trust and stopping. Reading uncertainty off the physics is a decision and
