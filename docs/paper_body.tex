...
difference of per-ligand offsets, and no statistic computed from the network's own edge values can
tell it from no error at all. The splitting itself is a known decomposition of edge flows on a
graph. What is not is where its two halves fall: the invisible part is exactly what biases the
estimated affinities, and the auditable part biases nothing, so a closure test is blind where an
error would matter and sighted where it would not. Resolving it needs measurements from outside
the network, and we price that in ligands. Judging the audit quantitatively needs a scale. The
Bennett Acceptance Ratio carries its finite-sampling variance in closed form, and the per-edge bar
a practitioner already receives is checked here against independent molecular-dynamics replicates
on many real protein-ligand binding edges. As the null of a cycle-closure fit it makes the check a
test with a stated level, where an overconfident stand-in flags almost everything. On the public
benchmark examined here the auditable subspace spans a third of the error space. On systems
grounded against public affinity data it carries, as an upper bound in the variance-weighted
metric, under one per cent of the error present. The share sits below chance on every system and
in every metric tried, by a margin that ranges from wide to slim. Per-edge
