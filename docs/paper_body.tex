\begin{abstract}
The per-edge error bar a relative-binding-free-energy pipeline already reports is checked here
against seed and velocity repeats on more than a thousand real protein-ligand binding edges, and is
conservative on aggregate, heterogeneous by system. Used as the null of a cycle-closure
fit it turns a descriptive consistency check into a test with a stated null. A stand-in bar several
times too small destroys that test; a same-budget estimator pooling by overlap does not. Against the
fixed hysteresis cutoff the field uses, the flag sets are strictly nested: the calibrated null
catches nothing that cutoff misses and only removes a fifth of its flags, which retrospective data
cannot adjudicate. What the test can reach is fixed by a
decomposition of edge flows into gradient and cycle parts, which is not ours and is attributed. Its
halves fall awkwardly: no statistic computed from the network's own edge values can tell the
gradient part from no error at all, and that is the part biasing the estimated affinities, while
the auditable cycle part biases nothing. We measure where a real
network's error falls between them. The auditable subspace spans a third of the error space, and on
systems grounded against public affinity data it carries under one per cent of the error against
experiment, below chance on every system and in every metric tried by a margin ranging from wide to
slim. Removing the edges the test names does not recover accuracy against experiment, nor does
removing those a rule derived from the decomposition prefers, which on one system does measurable
harm. Resolving the invisible part needs measurements from outside the network, priced here in
ligands.
\end{abstract}
...
\item An applicability map across interval sharpness, edge ranking, synthesis-commit selection and
reward-guided re-ranking. The calibrated uncertainty is valuable for detection and for trust in a
commit decision on real out-of-distribution benchmark edges, and for a stopping rule in simulation;
for nothing sharper or better ranked.
...
not re-rank what to compute or what to commit, and does not steer reward re-ranking on the one
evaluable congeneric set. It pays on commit trust, measured on real out-of-distribution benchmark
edges, and on a stopping rule in simulation; and an overconfident $\sigma$ silently corrupts both
decisions. Calibration is a trust instrument rather than a performance multiplier, and the graph
weight earns its place through structure rather than magnitude, making the gauge directions exactly
