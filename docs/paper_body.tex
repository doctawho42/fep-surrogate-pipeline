...
was meant to replace: better on one system of four, at a pre-registered sign test of $p=0.94$
against the residual rule's $0.69$ (Figure~\ref{fig:Hodge}C). The pre-registration's other
combination, a one-sided Stouffer, returns no usable number here, because one system's permutation
$p$ saturates at the resolution of its own grid and Stouffer maps that to an infinite $z$; clipping
to the grid's range gives $0.96$, and the residual rule, which has no saturated input, stands at
$0.48$. Neither rule beats random removal, and neither is shown to be worse than it. The reason is the same measurement: the influence form divides a
...
Figure~\ref{fig:Lcausal}) and the influence rule does no better (sign test $p=0.94$;
