...
a selective minority of perturbation networks as carrying inconsistency that reproduces across
independent runs. What the test
identifies reproduces out of sample: miscalibration measured on one set of replicates predicts the
closure statistic of a replicate it never saw. Those runs share their starting coordinates, so what
is established is reproducibility beyond sampling noise and not physical error. The ranking behind
the flags is steadier than the
list of names flagged at a fixed threshold, and we report both. A stand-in for that error bar,
built edge by edge from a learned head's measured overconfidence rather than by running that head
on this benchmark, destroys the
...
baseline anyone reports. Figure~\ref{fig:A}B checks the same calibration on work samples from a
real protein--ligand system: the $19$ adjacent-$\lambda$ BAR windows of the complex leg of the
\texttt{alchemtest} BACE1 relative-binding calculation, drawn from its decharge, recharge and van
der Waals legs, which share one $\lambda$ schedule (sandwich over bootstrap truth $1.00$,
$[0.96,1.07]$); this bootstrap
reference reuses the same work samples (an internal consistency check). On those same windows the
naive $1/I$ overstates the bootstrap se by $2$--$12\times$ (mean $6.07$, $[2.78,12.01]$), with
every window sitting at overlap $\geq0.84$ (per-window values, Supplementary
Table~\ref{tab:bace1}). A window is one two-state BAR estimate between neighbouring $\lambda$
values and not a ligand-to-ligand binding free energy, so what this panel establishes is that the
closed form matches a decorrelated bootstrap on real molecular-dynamics works, in the
high-overlap regime this schedule occupies; the low-overlap behaviour is the controlled sweep of
Figure~\ref{fig:A}A, and the per-transformation binding quantity is Figure~\ref{fig:Arep}.
...
cycle graph itself. On the same $1143$ edges we ran a pre-registered head-to-head of the
calibrated per-edge null against the fixed $1.0$ kcal/mol hysteresis cutoff the field uses and
against a fixed-$\mathrm{se}$ $\chi^2$ test built on the same statistic, scored by area under the
receiver-operating-characteristic curve (AUC) against the reproducible-systematic magnitude of
Section~\ref{sec:methods} (calibrated $0.865$, fixed cutoff $0.575$, fixed $\mathrm{se}$ $0.725$;
paired bootstrap differences $+0.290$ $[+0.094,+0.490]$ and $+0.140$ $[-0.008,+0.316]$;
Supplementary Figure~\ref{fig:Cut}). The pre-registered criterion required beating both, so the
verdict was and remains a tie. We withdraw the further reading we placed on the ordering inside
it, that the calibrated null discriminates better than the fixed cutoff. The only label these
data supply divides each residual by the same per-edge reported variance the calibrated rule uses
as its weight, so a rule scores well here for standing close to that variance, and the AUC
ordering is monotone in that proximity rather than in detection skill. The comparison is
contaminated and ranks nothing. What the calibrated null has over the fixed cutoff is accordingly
structural and not measured by this comparison, namely per-edge-adaptive thresholds, a
false-positive rate tied to a replicate-validated null, and the observability map of
Theorem~\ref{thm:cons}. The evidence that
...
two-sided counterparts and with each arm's realized probability of a false flag under the
pure-null simulation of Section~\ref{sec:methods}; among the arms whose realized level was measured, the pre-specified per-edge
...
model that turns FEP cycle-closure into a quality-control test that separates reproducible from sampling inconsistency on real networks and whose output reproduces on replicates it never saw, the sharpest use of the free calibration, and one a bar as overconfident as the
learned $\sigma$ measured here destroys.
...
for the rest. The flags themselves come mostly from the other end of that distribution, seven of
the eight systems ever flagged and the one flagged on every repeat being well-connected, so the
churn is largely the low-cycle tail moving in and out rather than the detector disagreeing with
itself where it has power. The eighth is \texttt{brd4}, the one ever-flagged system carrying a
single cycle, and low connectivity does not make it a borderline detection. Its reduced $\chi^2$
of $15.40$ on the first replicate is the largest of the $48$ systems, it is flagged on two of the
three repeats ($8.05$, then $4.94$ unflagged), and it remains the largest after being retested
against the bar width its own replicates measure ($8.97$, Supplementary Table~\ref{tab:P8}). One
degree of freedom costs it the stability of its adjusted $p$-value, not the size of its
inconsistency.
...
line, which is what any reproducible offset does: it inflates a closure residual
without inflating the spread between repeats. Being reproducible across repeats that share a
...
effect against its baseline. Three of the performance tasks, interval sharpness,
commit-to-synthesis selection, and reward re-ranking over a measured congeneric set (single
target \texttt{eg5}), run on the public OpenFF protein--ligand
benchmark~\citep{hahn2022plbench} (FEP+ and Merck data~\citep{wang2015fep,schindler2020merck};
experimental $\ddG$ labels, both-endpoint scaffold-disjoint out-of-distribution (OOD) splits), as
does the commit-trust decision task; the fourth performance task, edge ranking in active
learning, and the stopping decision task both run in controlled simulations. The amortized $\sigma$ under test (the decomposed
physics $\sigma$) is the trunk's \emph{predictive} uncertainty on unseen $\ddG$ values,
$\sigma_{\text{total}}=\gamma\sqrt{\sigma^2_{\text{ens}}+\sigma^2_{\text{ale}}}$: deep-ensemble
epistemic spread and benchmark aleatoric floor, conformally scaled at miscoverage $\alpha=0.1$
(90\% target coverage; the scalar $\gamma$ rather than $q$, reserved throughout for a
Benjamini--Hochberg adjusted $p$-value). It is not Figure~\ref{fig:A}'s per-edge BAR sampling-se
or its learned-variance foil; a bare ``$\sigma$'' below means $\sigma_{\text{total}}$.

\paragraph{Trust: an overconfident $\sigma$ corrupts decisions; this decomposed $\sigma$ does
not, and calibration is cheap (on the OpenFF benchmark).} Supplementary Figure~\ref{fig:J}B fixes
the (ensemble) mean and varies only the uncertainty in the commit rule (commit to synthesis iff
the lower-confidence bound (LCB) $\mu-\kappa\sigma\ge\tau$, $\kappa=z_{1-\alpha}$). An
overconfident epistemic-only $\sigma$ over-claims badly: its realized-versus-claimed correctness
shortfall is $+0.182$ (CI $[0.132,0.225]$) larger than the decomposed $\sigma$'s, which is
calibrated-to-conservative. The trust is \emph{not unique} to the physics $\sigma$: temperature
scaling~\citep{guo2017calibration} or split-conformal recalibration of the same overconfident
$\sigma$ recovers it (temperature$-$ours $+0.034$, CI $[-0.018,0.081]$; conformal$-$ours
$-0.029$, CI $[-0.077,0.007]$; both indistinguishable from ours). Trustworthy decisions need
\emph{some} proper calibration; the physics $\sigma$ supplies it at no calibration-set cost for
the aleatoric term.

\paragraph{Sharpness: this decomposed $\sigma$ is not sharper on real out-of-distribution
residuals (OpenFF benchmark).} On \emph{real} held-out trunk residuals (Supplementary
Figure~\ref{fig:Breal}) at matched $\sim$0.90 marginal coverage, the decomposed interval is
slightly \emph{wider} than flat split-conformal (width ratio split/ours $0.92\times$): epistemic
spread is small against the large irreducible OOD error (half-width correlates with
$|$residual$|$ only $+0.11$). Its conditional calibration is marginally better than a flat band,
comparable to a fair Mondrian-by-target conformal (per-target max coverage error $0.17$ vs $0.22$
for split and Mondrian alike), but the advantage is not significant (ours$-$split $-0.044$, CI
$[-0.121,0.005]$). The supplement's synthetic 1-D illustration, whose generative structure the
decomposition knows, is up to $2.2\times$ sharper than split-conformal and $2.0\times$ than CQR;
that does not transfer to real data.

\paragraph{Ranking and selection: the choice of $\sigma$ is second-order at matched budget.} For
edge ranking in active learning, sandwich versus naive weights differ by $0.98\times$
$[0.93,1.03]$ on the primary calls-to-top-$k$ metric (24-seed bootstrap CI; Figure~\ref{fig:C},
supplement): in a connected graph the GLS contrast mean is unbiased for \emph{any} positive
weights (Gauss--Markov), so the weight choice is second-order for ranking. So too for commit
selection: at matched commit \emph{volume} (Supplementary Figure~\ref{fig:J}A, ranking by the
standardized margin $(\mu-\tau)/\sigma$ and committing the top fraction), the $\sigma$-aware
ranking does not beat a raw-$\mu$ ranking (precision at a 25\% budget $0.674$ vs $0.680$;
trunk$-$raw $-0.006$, CI $[-0.016,0.007]$). A calibrated bar says \emph{whether} to act, not
\emph{which} action ranks first.

\paragraph{Reward re-ranking (evaluable congeneric set, single target \texttt{eg5}): this
decomposed $\sigma$ does not help, and where $\sigma$ varies it hurts.} A trajectory-balance
GFlowNet~\citep{bengio2021gflownet,malkin2022tb} samples a held-out target's measured congeneric
series (trunk trained target-disjoint) under three rewards: the calibrated LCB, the raw predicted
value, and an overconfident MVE reward. On \texttt{eg5}, the on-disk target with the
\emph{largest} $\sigma$ spread, so that the calibrated reward genuinely re-ranks candidates, it
\emph{lowers} the true hit-rate of the sampled distribution ($0.140$ vs raw $0.249$ and MVE
$0.255$; cal$-$raw $-0.109$, CI $[-0.150,-0.072]$; Supplementary Figure~\ref{fig:K}). The
high-$\sigma$ ligands are the chemically novel \emph{strong} binders
($\mathrm{corr}(\sigma_{\text{total}},\text{affinity})=+0.42$), so the LCB penalty
$-\kappa\sigma$ demotes the true winners. This is the best case available, and it fails. Resting
on one evaluable target, it is a \emph{hypothesis}, not a transferable diagnostic: an LCB reward
may help only where $\sigma$ flags \emph{low quality}; where it flags \emph{novelty}, as on this
congeneric series, the bound penalizes exactly the candidates worth making, and an uncalibrated
reward is safer.

\paragraph{Where it pays: the stopping decision.} Supplementary Figure~\ref{fig:G} runs two
active-learning loops sharing a correct (true-precision GLS) posterior mean but differing in
assumed uncertainty (a simulated posterior variance, not $\sigma_{\text{total}}$ above): the
calibrated learner's claimed confidence tracks actual top-$k$ correctness ($|\Delta|=0.022$) and
stops at $30$ calls $82\%$ correct, while an overconfident learner (its assumed posterior se
scaled by $0.15\times$, the ${\sim}7\times$ central overconfidence \emph{measured} by
Figure~\ref{fig:A}'s sampling-se foil and reused here only as a numeric shrink factor) stops
twice as early at only $60\%$ correct. This is a controlled simulation, but the lesson of the
commit panel again.

\paragraph{Synthesis.} Six independent tests agree: a calibrated per-edge $\sigma$ does not
sharpen intervals on real data, does not re-rank what to compute, does not re-rank what to commit,
and does not steer
reward re-ranking toward better molecules on this evaluable congeneric set (single target
\texttt{eg5}); it pays on commit trust and on stopping; and an \emph{overconfident} $\sigma$
silently corrupts the
stop/commit decision. Calibration is therefore a \emph{trust/decision instrument}, not a
performance multiplier: what the physics variance buys is a calibrated decision, an identifiable
acquisition, and above all the quality-control detector of Section~\ref{sec:qc}, not a sharper or
better-ranked prediction. The graph weight earns its place through \emph{structure}, not
magnitude: it makes the gauge directions exactly unidentifiable (knowledge gradient $=0$) and
supplies the trust calculation, though by Gauss--Markov its numerical \emph{value} does not
improve ranking.
...
column of Supplementary Table~\ref{tab:selfgrid}, is measured on a synthetic world in which
nothing is wrong, so that every flag it produces is a false positive by construction. The
...
per-edge reported standard error it ships with, and that value is then \emph{declared} to be the
...
heterogeneous per system: of the $34$ systems carrying at least eight edges, $28$ are conservative
and the remaining six are overconfident, namely \texttt{t4\_lysozyme} $0.99\times$,
\texttt{hif2a} $0.96\times$, \texttt{bace} $0.96\times$, \texttt{faah} $0.90\times$, \texttt{brd4}
$0.76\times$, and the protonation variant \texttt{bace\_p3\_arg368\_in} $0.41\times$, which is the
only one of the six that is not marginal. The three replicates are seed/velocity-only repeats from identical starting
...
closer to neutral against real reproducibility. That same restriction limits what the
quality-control test can be said to detect. An inconsistency reproducing across repeats that
share one starting structure is established as more than sampling noise, but a fixed consequence
of that structure, a pose or a protonation assignment carried unchanged into all three, would
reproduce identically; the pooling argument of Figure~\ref{fig:L}C and the cross-replicate
residual correlation of Figure~\ref{fig:Lval}B are both invariant to which of the two it is, so
neither establishes physical error, and the flags are reported as reproducible inconsistency
throughout. The
...
not the cycle count $\nu$ of the closure statistic), and the pure-null simulation of
Section~\ref{sec:methods} puts the probability of at least one false
flag at $0.59$ for the two-sided per-edge arm and $0.24$ for the two-sided aggregate one, against
the $0.04$ the uncorrected arm realizes on that same simulation at the same nominal
$\alpha=0.05$; the one-sided per-edge arm is the only self-calibrated arm that holds its level
($0.003$).
