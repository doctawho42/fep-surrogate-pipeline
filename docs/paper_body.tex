...
metric, under one per cent of the error present, at a weakest margin over chance of two. Per-edge
...
$0.0086$ and $0.0460$, each stable to within a factor of three under leaving out any one edge. Both
...
near-bridge is divided by its own vanishing leverage. The rule fails for the reason measured above: the influence form divides a residual by a small
...
Figure~\ref{fig:Lcausal}) and the influence rule does no better overall and, on \texttt{p38},
measurably worse than random removal (Section~\ref{sec:hodge}; Figure~\ref{fig:Hodge}C).
