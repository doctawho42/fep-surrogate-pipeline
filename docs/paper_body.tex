...
$27\%$ can resolve one of $0.5$; the detectable-shift resolution of the median edge is the size of
...
should be tested across more chemotypes. The single-target re-ranking result is directional. The cycle-closure quality-control test (Section~\ref{sec:qc}) detects only \emph{edge-level} inconsistency (node-consistent force-field bias cancels around a loop and is invisible to it), and its flags are validated retrospectively against held-out replicates and against experiment both on one clean system (\texttt{eg5}, where acting on the flags neither helps nor harms) and, now, on four flagged systems grounded against public ChEMBL affinity (\texttt{cdk8, hif2a, p38, bace}), where guided QC-edge removal ties random removal on MUE-vs-experiment (combined Stouffer $p=0.48$; Figure~\ref{fig:Lcausal}), and where a pre-registered second rule ranking edges by influence rather than by residual does worse still ($p=1.00$; Figure~\ref{fig:Hodge}C). Theorem~\ref{thm:hodge} says both nulls are structural rather than underpowered, and Section~\ref{sec:hodge} measures the margin: under one per cent of the error against experiment lies where a removal rule could act on it. Three restrictions attach to that measurement. It rests on four systems, all of them flagged, so it characterises the systems the test selects rather than the benchmark. It runs on a connectivity-based ChEMBL join (stereo-blind, so enantiomer pairs sharing a 2D graph collapse to one experimental value, inflating absolute MUE but canceling in the guided-vs-random contrast since both arms read the identical cached affinity), and the experimental and annotation errors it carries are per-ligand quantities that enter the same unauditable subspace as force-field bias, so the measurement locates the error without attributing it. And the influence rule is a second pre-registered hypothesis on the data that returned the first null, reported as a second look rather than as independent confirmation. The theorem itself bounds only what a network's own edge values can support, and says nothing against methods that bring in information from outside the network. The fully prospective flag$\to$repair$\to$remeasure test on a flagged system, with new MD rather than a retrospective affinity join, lies outside what public data can support. The distribution-free cross-replicate check establishes that what is flagged reproduces across independent runs, but it is not a quantitative false-positive-rate guarantee: it does not certify the nominal Benjamini--Hochberg rate itself. The detector's \emph{recall} cannot be measured directly, since there are no ground-truth error
