...
what any consistency check can do. One part lives in the network's cycles; the other is a
difference of per-ligand offsets that no statistic computed from the network's own edge values can
tell from no error at all. The splitting itself is a known decomposition of edge flows on a
graph. What is not is where its halves fall: the invisible part biases the
estimated affinities and the auditable part biases nothing, so a closure test is blind where an
error would matter. Resolving it needs measurements from outside the network, priced here in
ligands. Judging it quantitatively needs a scale. The Bennett
Acceptance Ratio carries its finite-sampling variance in closed form, and the per-edge bar a
practitioner already receives is checked here against seed and velocity repeats on many real
protein-ligand binding edges. As the null of a cycle-closure fit it makes the check a
test with a stated null. A stand-in bar several times too small destroys it; a same-budget
estimator pooling by overlap does not. On the public
benchmark examined here the auditable subspace spans a third of the error space. On systems grounded
against public affinity data it carries, in the variance-weighted metric, under one per cent of
the error present. The share sits below chance on every system and in
every metric tried, by a margin ranging from wide to slim. Per-edge
selections and the ranking of systems reproduce on replicates they never saw; the flagged list
does not. Removing flagged edges does not recover accuracy against experiment, and on one system a
rule derived from the split does measurable harm.
\end{abstract}
...
networks~\citep{geiger2022e3nn,batzner2022nequip,batatia2022mace} are the de-facto trunks for 3D
molecular property prediction, where parity-\emph{odd} irreps to break achirality are standard.
...
\emph{(iv)} Given unbiased absolute measurements on a set $S$ of ligands, the unidentifiable
dimension is
...
Three things are added here. The first is the reading: HodgeRank estimates the gradient component,
so it has no reason to ask what an error lying along that component does to the estimate. Part (ii)
asks and answers it, and the answer is not the neutral one a reader might assume: the bias and the
detectability are carried by complementary halves exactly, so a closure test is blind precisely
where it would matter and sighted precisely where it would not. The second is part (iv), which
prices the escape in measurements rather than leaving it qualitative. The third is the measurement
of where a real perturbation network's error actually falls between the two halves, which is
Section~\ref{sec:hodge} and is what the rest of this paper rests on. Parts (i) and (iii) are
restatements in the vocabulary of alchemical networks, and we report them as such because the
measurement is not interpretable without them.
...
Information). What these support is a bound and not a point estimate: the auditable share is small,
...
can carry no evidence at all. It does not deliver a repair list: removing the edges it names does not recover accuracy against
